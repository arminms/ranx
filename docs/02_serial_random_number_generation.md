...
[next chapter »](./03_parallel_random_number_generation.md).
::::
...
And the common practice was to use *modulo operator* (`%`), like [the way we did](./01_randomness_primer.md#randogram) in the previous chapter, to bring them into a range:
...
That’s especially important for passing engines with huge [footprint](./01_preliminary_concepts.md#arng_attributes) like <wiki:Mersenne_Twister>, but as not much for the distributions.
:::
